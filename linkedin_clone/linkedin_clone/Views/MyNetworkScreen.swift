...
struct MyNetworkScreen: View {
    ...
    var body: some View {
        VStack(){
            SearchBarView()
            HStack(){
                Text("Manage my network").font(.body).foregroundColor(.blue).fontWeight(.bold)
                Spacer()
                Image(systemName: "chevron.right").foregroundColor(.gray)
            }.padding(.horizontal)
            HorizontalDivider(color: .gray, height: 10).opacity(0.3).ignoresSafeArea(.all)
            
            
            ScrollView(.vertical){
                HStack(){
                    Text("Invitations").font(.body).foregroundColor(.blue).fontWeight(.bold)
                    Spacer()
                    Image(systemName: "chevron.right").foregroundColor(.gray)
                }.padding(.horizontal)
                Divider()
                ForEach((networkData), id: \.id){ data in
                      InvitationView(profileData: data)
                    Divider()
                   }
            }
        }
